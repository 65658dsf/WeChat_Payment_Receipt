...
- 识别到支付成功后立即向 webhook 发送带签名通知，再点击目标订单卡片主体进入收款记录，完成关闭、确认关闭、删除和确认删除。订单卡片点击支持最多 3 个主体位置重试，并同时使用 UIA 标题和窗口画面变化确认切页，避免小程序未暴露“收款记录”文本时误报失败。
...
- 关闭和删除收款单的快速模式只读取一次窗口矩形，缓存 6 个操作的绝对坐标，并用 0.20 秒过渡等待连续完成关闭和删除；最终删除后等待 0.40 秒，避免每一步耗时约 2 秒的 UIA 查询。
