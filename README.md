...
- 支付成功后进入收款记录，关闭收款单、确认关闭、删除收款单、确认删除。
- 支付成功后向 webhook 发送带签名通知。
- Webhook 成功后删除本地二维码图片。
...
- API 服务创建并保存二维码截图、完成 base64 编码后会立即返回 `/create`，返回主界面和等待支付在后台继续执行。`/create` 最多同步等待 10 秒，未完成时返回 `code=2` 与 `retry_after_seconds`，客户端等待后重试同一订单；`POST /estimate` 可查询队列位置和预计时间。服务一次只操作一个订单流程，相同订单号会复用活跃任务和二维码。
- `outputs/` 和 `keys/*.pem` 已忽略提交。
- Webhook 发送成功后才会删除本地二维码图片；如果 webhook 失败，图片会保留用于排查。
