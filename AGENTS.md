...
- 创建收款单完成后会读取“生成收款码”UIA 元素的实时矩形，并真实点击元素中心进入“生成分享图”页面；这里不使用可能无效果的 UIA Invoke。进入页面后按收款码裁剪区的暗色像素占比等待二维码纹理连续两帧达到阈值，再保存到 `outputs/`；“加载中”占位图或最终复检不合格的截图必须删除并按 `generate_qr_retry_count` 重试，不能编码返回。截图保存后默认点击左上角返回两次，回到主界面等待付款列表；检测到“已支付”后先发送成功 Webhook，再进入详情关闭、确认、删除收款单。达到支付等待上限时先发送失败 Webhook，再进入关闭删除流程。关闭、确认、删除和更多操作默认必须读取并操作对应 UIA 文本元素，找不到元素时停止，不能使用可能落到“发起收款”的固定坐标兜底；最终确认目标订单号已从列表消失后才返回删除成功。
- 等待支付时通过右上角三个点打开小程序菜单，再点击“重新进入小程序”刷新页面；普通检查优先复用矩形仍有效的 UIA `root`，每次重新进入后必须丢弃旧 `root` 并按窗口标题获取新对象，避免窗口重建或 PID 变化导致旧对象失效。重新进入后必须等待读取到“暂无人付款”或“已支付”才能继续；仅有页头、订单号或其他页面文本时仍视为状态未知。若 `order_status_load_timeout_seconds` 内未读到明确状态，应等待 `status_refresh_retry_wait_seconds` 后再次执行“重新进入小程序”。状态识别按 `order_status_uia_max_depth`（默认 16）遍历 UIA 树，优先用订单号和状态文本的独立矩形匹配；刷新后如果微信把卡片合并成单个 UIA 容器，则匹配同时包含目标订单号和状态的非整页最小容器。微信列表没有暴露订单号时，仅在页面只出现一种明确状态时兜底：只有“已支付”按已支付，只有“暂无人付款”按未支付；两种状态同时出现或只有其他页面文本仍为未知。状态点击点优先选择面积最小且不覆盖整窗的文本矩形。达到 `max_payment_refresh_count`（默认 5）次时，无论明确未支付还是状态未知，均判定支付失败，发送 `TRADE_FAILED` Webhook 后进入订单详情关闭并删除收款单，清理失效二维码后释放任务。
...
- Webhook 回调使用另一组全新的密钥：服务端用 `keys/webhook_public_key.pem` 加密 `trade_no+total_amount+trade_status` 生成 webhook `sign`；客户端用 `keys/webhook_private_key.pem` 解密并比对。`trade_status` 为 `TRADE_SUCCESS` 表示已支付，`TRADE_FAILED` 表示达到刷新上限后支付失败，`TRADE_ERROR` 表示读取或刷新订单状态时发生异常。状态异常时必须先回调，再继续尝试关闭删除并释放任务。
- `/create` 请求验签通过后进入单线程订单队列。截图和 base64 编码完成后立即设置 `created_event`；返回主界面、等待支付、Webhook 和图片清理由后台继续执行。`/create` 只同步等待 `create_request_wait_seconds`，未完成时返回 HTTP 200、`code=2`、预计时间和 `retry_after_seconds`，客户端等待后用同一订单号重试。成功响应的 `data.reused` 表示是否复用了同订单号的活跃任务。`POST /estimate` 使用相同签名返回预计时间但不创建任务。已有订单等待支付时新订单继续排队，相同订单号复用已有任务和二维码。
