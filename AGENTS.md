...
- 创建收款单完成后会点击“生成收款码”，进入“生成分享图”页面，并按相对比例裁剪中间白色收款码卡片保存到 `outputs/`。截图保存后默认点击左上角返回两次，回到主界面等待付款列表；可继续每秒刷新一次，检测到“已支付”后进入收款记录详情，点击“更多操作”并关闭收款单，确认关闭后再次点击“更多操作”并删除收款单。
