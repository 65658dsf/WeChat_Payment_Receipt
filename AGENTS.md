...
- 当前微信小程序页面内的金额/说明输入框不稳定暴露为标准 `Edit` 控件。金额和订单号输入会先按标签附近的输入控件读取并点击，找不到标准控件时再使用窗口相对坐标；金额数字仍通过小程序数字键盘坐标输入，订单号通过剪贴板粘贴。兜底坐标比例维护在 `utools/components/wechat_pay_order.py`。
...
- 创建收款单完成后会点击“生成收款码”，进入“生成分享图”页面，并按相对比例裁剪中间白色收款码卡片保存到 `outputs/`。如果点击后没有进入“生成分享图”，会按 `generate_qr_retry_count` 重新获取窗口并重试。截图保存后默认点击左上角返回两次，回到主界面等待付款列表；可继续每秒刷新一次，检测到“已支付”后进入收款记录详情，点击“更多操作”并关闭收款单，确认关闭后再次点击“更多操作”并删除收款单，最后确认删除。
