...
如果多次请求使用相同的 `pid`，并且该订单仍在排队、创建中或等待支付中，服务端会直接复用该订单，不会重复创建微信收款单；请求会返回同一个订单号对应的二维码。状态识别时，只要页面能读到有效文本且不包含“暂无人付款”，即按已支付通过。支付成功后会优先点击目标订单号所在的卡片主体；若未切页，会在同一卡片内尝试其他主体位置，并通过 UIA“收款记录”标题或窗口画面变化确认已经进入详情。等待支付期间连续重新进入小程序达到 5 次且明确仍为“暂无人付款”时，服务端不会发送成功 Webhook，而是进入订单详情关闭并删除收款单，清理失效二维码后释放后台任务；页面完全读不到文本时继续刷新。后续使用相同 `pid` 调用 `/create` 会重新创建收款单。
